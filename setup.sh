...
### CAUSION : valid only for 32bit raspi OS Lite
###           pigpio is not available via apt in 64bit OS
...
cat << EOS | sudo tee -a /boot/firmware/config.txt
dtparam=pwr_led_trigger=heartbeat
EOS

# Activate i2c
echo ">>> Activate i2c\n"
sudo raspi-config nonint do_i2c 0 || echo "EROOR at i2c activate" >> log.txt
